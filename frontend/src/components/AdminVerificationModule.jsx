...
import { CaretLeft, CheckCircle, DownloadSimple, UserCirclePlus, UsersThree, ShieldCheck, Bell, FileText } from '@phosphor-icons/react';
import { getLeadersForProgramme } from '../config/userRegistry';
import { apiSaveApplication } from '../utils/db';
...
const AdminVerificationModule = ({ applications, setApplications, leaders = [], initialTab = "new", verifyAppId, setVerifyAppId }) => {
  ...
  React.useEffect(() => {
    if (verifyAppId) {
      const app = applications.find(a => (a.id === verifyAppId || a._id === verifyAppId) && verifyAppId);
      if (app) {
        setViewingApp(app);
  ...
  const handleVerify = async (id) => {
    ...
    const appToVerify = applications.find(a => (a.id === id || a._id === id) && id);
    setNotification(`📨 Notifying ${selectedLeader} via secure channels...`);

    try {
      // 1. Trigger Backend Email Notification
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/notify-leader`, {
        method: 'POST',
      ...
      const notifKey = `ibes_notif_${selectedLeader.trim().toLowerCase()}`;
      ...
      const updatedApp = { ...appToVerify, currentStep: 3, status: "pending", assignedLeader: selectedLeader };
      await apiSaveApplication(updatedApp);

      setApplications((prev) =>
        prev.map((app) =>
          ((app.id === id || app._id === id) && id) ? updatedApp : app
        )
      );
      ...
      setApplications((prev) =>
        prev.map((app) =>
          ((app.id === id || app._id === id) && id) ? { ...app, currentStep: 3, status: "pending", assignedLeader: selectedLeader } : app
        )
      );
  ...
  const handleDirectApprove = async (id) => {
    const appToApprove = applications.find(a => a.id === id || a._id === id);
    const updatedApp = { ...appToApprove, currentStep: 4, status: "approved" };
    await apiSaveApplication(updatedApp);
  ...
  const handleReject = async (id) => {
    if (window.confirm("Are you sure you want to permanently reject this application?")) {
      const appToReject = applications.find(a => a.id === id || a._id === id);
      const updatedApp = { ...appToReject, status: "rejected" };
      await apiSaveApplication(updatedApp);
    ...
    return (
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '12px' }}>
        {files.map((file, i) => {
          ...
          const isPlaceholder = !isUrl && file && file.isFilePlaceholder;
          const isRealFile = !isUrl && file instanceof File;
          
          const name = isUrl ? file.split('/').pop() : (file.name || "Unknown File");
          const url = isUrl ? file : (isRealFile ? URL.createObjectURL(file) : null);
          const isImage = isUrl ? (/\.(jpeg|jpg|gif|png|webp)$/i.test(file)) : (file.type || "").startsWith('image/');
          ...
          return (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', border: '1px solid #e2e8f0', borderRadius: '8px', backgroundColor: 'white', transition: 'all 0.2s', boxShadow: '0 1px 2px rgba(0,0,0,0.02)' }}>
              {isImage && (url) ? (
                <div style={{ width: '40px', height: '40px', borderRadius: '4px', overflow: 'hidden', flexShrink: 0, border: '1px solid #e2e8f0' }}>
                   <img src={url} alt={name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                </div>
              ) : (
                <div style={{ width: '40px', height: '40px', backgroundColor: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '4px', border: '1px solid #e2e8f0', color: '#94a3b8' }}>
                  <FileText size={20} />
                </div>
              )}
              <div style={{ flexGrow: 1, minWidth: 0 }}>
                <p style={{ margin: 0, fontSize: '13px', fontWeight: '600', color: '#1e293b', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{name}</p>
                <p style={{ margin: 0, fontSize: '12px', color: '#64748b' }}>
                  {isUrl ? "Stored on Server" : (file.size ? `${(file.size / 1024).toFixed(1)} KB` : "Local File")}
                  {isPlaceholder && <span style={{ color: '#ef4444', marginLeft: '6px' }}>(Cleared on refresh)</span>}
                </p>
              </div>
              {(isRealFile || isUrl) ? (
                <a href={url} download={name} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '32px', height: '32px', borderRadius: '50%', backgroundColor: '#f1f5f9', color: 'var(--ibes-navy)', flexShrink: 0, transition: 'background 0.2s' }} title="Download File">
