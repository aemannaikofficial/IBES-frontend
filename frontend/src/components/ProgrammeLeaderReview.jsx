...
const ProgrammeLeaderReview = ({ applications, setApplications, userRole, userName, handleLogout }) => {
  ...
  useEffect(() => {
    if (!userName) return;
    const notifKey = `ibes_notif_${userName.trim().toLowerCase()}`;
  ...
  const pendingApps = applications.filter(app => {
    const isStep3 = app.currentStep === 3;
    const isLeaderMatch = app.assignedLeader?.trim().toLowerCase() === userName?.trim().toLowerCase();
    const isAdmin = userRole === 'admin';
    if (isStep3) {
      console.log(`[DEBUG] Leader Queue check for ${app.fullName}:`, { 
        isStep3, 
        assigned: app.assignedLeader, 
        currentLeader: userName, 
        match: isLeaderMatch, 
        isAdmin 
      });
    }
    return isStep3 && (isAdmin || isLeaderMatch);
  });
...
const RegistryManagementView = ({ type, applications, registryFilter, setRegistryFilter, setViewingApp, handleDelete, userName }) => {
  ...
  const filteredApps = applications.filter(app => {
    ...
    const isLeaderMatch = app.assignedLeader?.trim().toLowerCase() === userName?.trim().toLowerCase();
    return isTargetType && (isLeaderMatch || !app.assignedLeader);
  });

  // removed unused availableProgrammes
