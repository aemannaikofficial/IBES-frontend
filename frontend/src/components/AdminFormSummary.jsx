...
const AdminFormSummary = ({ applications, setApplications, type, setActiveView, setVerifyAppId, intakes = [], modules = [] }) => {
  ...
  const filteredApps = applications.filter(app => {
    ...
    if (type === "Leader Module") {
      return app.applicationType === "Leader Module" || app.applicationType === "Leader" || app.applicationType === "Module Leader";
    }
  ...
  console.log(`[DEBUG] AdminFormSummary (${type}):`, { total: applications.length, filtered: filteredApps.length, displayed: displayApps.length, activeFilter });

  const handleUpdateApp = (e) => {
    e.preventDefault();
    const editingId = editingApp.id || editingApp._id;
    if (!editingId) {
      console.error("No ID found for application update");
      return;
    }
    setApplications(prev => prev.map(a => (a.id === editingId || a._id === editingId) ? editingApp : a));
    setEditingApp(null);
  };
                  ...
                  <tr key={idx} style={{ borderBottom: '1px solid #f1f5f9', transition: 'all 0.2s', backgroundColor: idx % 2 === 0 ? 'white' : '#fafafa' }}>
                    ...
                    <td style={{ padding: '16px 32px', textAlign: 'right' }}>
                      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
                        {/* Generate Contract button — Leader Module + Approved only */}
                        {isLeader && app.status === 'approved' && (
                          <button
                            ...
                            title="Generate Module Leader Contract"
                          >
                            📄 Contract
                          </button>
                        )}
                        {/* Download PDFs button — Teacher Applicant + Approved only */}
                        {!isLeader && app.status === 'approved' && (
                          <button
                            onClick={async () => {
                              try {
