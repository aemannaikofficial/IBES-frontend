...
import { UploadSimple, UserCircle, Briefcase, GraduationCap, FileText, PenNib, CircleNotch } from "@phosphor-icons/react";
...
const TeacherApplicantForm = ({ onSubmit, leaders, isGeneral, programmes = [] }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // --- 📤 Upload Files ---
      // Helper to upload a field and return URLs
      const uploadField = async (files) => {
        if (!files || (Array.isArray(files) && files.length === 0)) return [];
        const fileArr = Array.isArray(files) ? files : [files];
        return await apiUploadFiles(fileArr);
      };

      const [profileUrls, resumeUrls, idUrls, certUrls, transUrls] = await Promise.all([
      ...
      finalData.profilePicture = profileUrls[0] || null;
      finalData.resumeCV = resumeUrls;
      finalData.idPassport = idUrls;
      finalData.certificates = certUrls;
      finalData.transcripts = transUrls;
      ...
      try {
        await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/notify-form-distributed`, {
          method: 'POST',
