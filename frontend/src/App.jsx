...
import AdminIntakeManagement from "./components/AdminIntakeManagement";
...
import { saveToDB, getFromDB, apiSaveApplication, apiSaveLeader } from "./utils/db";
import { LEADER_REGISTRY, DEFAULT_PROGRAMMES, DEFAULT_MODULES, DEFAULT_INTAKES } from "./config/userRegistry";
...
function App() {
  ...
  const [applications, setApplications] = useState(() => {
    const stored = localStorage.getItem("ibes_apps");
  ...
  const [programmeModules, setProgrammeModules] = useState(() => {
    const stored = localStorage.getItem("ibes_modules");
    return stored ? JSON.parse(stored) : DEFAULT_MODULES;
  });

  const [intakes, setIntakes] = useState(() => {
    const stored = localStorage.getItem("ibes_intakes");
    return stored ? JSON.parse(stored) : DEFAULT_INTAKES;
  });
  ...
  useEffect(() => {
    const loadApps = async () => {
      // Load Applications
      const dbApps = await getFromDB('ibes_apps');
      console.log("[DEBUG] Fetched applications from DB:", dbApps);
      if (dbApps && dbApps.length > 0) {
        const normalizedApps = dbApps.map(app => ({
          ...app,
          id: app.id || app._id // Ensure 'id' exists for frontend components
        }));
        setApplications(normalizedApps);
        console.log(`[DEBUG] Updated state with ${normalizedApps.length} apps from MongoDB.`);
      } else {
        console.log("[DEBUG] MongoDB returned no apps, keeping local state.");
      }
      setIsAppsLoaded(true);

      // Load Leaders
      const dbLeaders = await getFromDB('ibes_leaders');
      if (dbLeaders && dbLeaders.length > 0) {
        setLeaders(dbLeaders);
      } else {
        // Seed MongoDB with initial leaders if empty
        console.log("Seeding MongoDB with initial leaders...");
        for (const leader of LEADER_REGISTRY) {
          await apiSaveLeader(leader);
        }
        setLeaders(LEADER_REGISTRY);
      }

      // Load Programmes
      const dbProgs = await getFromDB('ibes_programmes');
      if (dbProgs && dbProgs.length > 0) {
        setProgrammes(dbProgs);
      }
    };
    loadApps();
  }, []);
  ...
  useEffect(() => {
    localStorage.setItem("ibes_modules", JSON.stringify(programmeModules));
  }, [programmeModules]);

  useEffect(() => {
    localStorage.setItem("ibes_intakes", JSON.stringify(intakes));
  }, [intakes]);

  useEffect(() => {
    if (isAppsLoaded) {
      // Save full objects, including Files, to IndexedDB
      saveToDB('ibes_apps', applications);

      // Keep localStorage as a lightweight fallback (with placeholders)
      try {
        const serializableApps = applications.map(app => {
          const cleanApp = { ...app };
          for (const key in cleanApp) {
            if (Array.isArray(cleanApp[key])) {
              cleanApp[key] = cleanApp[key].map(item =>
                item instanceof File ? { name: item.name, size: item.size, type: item.type, isFilePlaceholder: true } : item
              );
            } else if (cleanApp[key] instanceof File) {
  ...
  }, [applications, isAppsLoaded]);

  const handleLoadSampleData = () => {
    const sampleData = [
      { id: "sample-1", fullName: "John Doe", ibesprogrammes: "Doctor of Business Administration (DBA) Mixed Mode", status: "pending", currentStep: 2, dateSubmitted: "01/09/2026", applicationType: "Teacher Applicant" },
      { id: "sample-2", fullName: "Jane Smith", ibesprogrammes: "Doctor of Education (EdD) Mixed Mode", status: "approved", currentStep: 4, dateSubmitted: "11/09/2026", applicationType: "Leader Module", assignedLeader: "Dr. Sarah Collins" },
      { id: "sample-3", fullName: "Dr. Robert Wilson", ibesprogrammes: "Doctor of Education (EdD) Mixed Mode", status: "pending", currentStep: 2, dateSubmitted: "09/09/2026", applicationType: "Teacher Applicant" }
    ];
  ...
  const handleFormSubmit = async (formData) => {
    console.log("[DEBUG] handleFormSubmit received:", formData);
    const newApp = {
      ...formData,
      currentStep: 2,
      status: "pending",
    ...
    console.log("[DEBUG] Prepared newApp:", newApp);

    // Save to MongoDB
    const savedApp = await apiSaveApplication(newApp);
    if (savedApp && !savedApp.error) {
      const normalizedSavedApp = { ...savedApp, id: savedApp.id || savedApp._id };
      setApplications((prev) => [...prev, normalizedSavedApp]);
    } else {
      // Fallback or Error
      if (savedApp && savedApp.error) console.error("Save Application Error:", savedApp.error);
      const fallbackApp = { ...newApp, id: Date.now() };
  ...
  const isAdminConsole = isLoggedIn && userRole === 'admin' &&
    ['dashboard', 'adminVerify', 'manageLeaders', 'programmes', 'leaderFormsSummary', 'tutorFormsSummary', 'programmeModules', 'intakes'].includes(activeView);
        ...
        <div style={{ display: 'flex', minHeight: '100vh', width: '100%', overflow: 'hidden' }}>
          ...
          <div style={{
            ...
            <div style={{ flex: 1, overflowY: 'auto' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                ...
                <button
                  onClick={() => { setActiveView('programmeModules'); if (isMobile) setIsSidebarOpen(false); }}
                  style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', backgroundColor: activeView === 'programmeModules' ? 'rgba(255,255,255,0.1)' : 'transparent', borderRadius: '10px', border: 'none', color: activeView === 'programmeModules' ? 'white' : '#94a3b8', cursor: 'pointer', textAlign: 'left', fontWeight: activeView === 'programmeModules' ? '600' : '500', fontSize: '15px', transition: 'all 0.2s' }}
                >
                  <List size={20} color={activeView === 'programmeModules' ? 'var(--ibes-red)' : 'currentColor'} weight={activeView === 'programmeModules' ? 'fill' : 'regular'} /> Programme Modules
                </button>

                <button
                  onClick={() => { setActiveView('intakes'); if (isMobile) setIsSidebarOpen(false); }}
                  style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', backgroundColor: activeView === 'intakes' ? 'rgba(255,255,255,0.1)' : 'transparent', borderRadius: '10px', border: 'none', color: activeView === 'intakes' ? 'white' : '#94a3b8', cursor: 'pointer', textAlign: 'left', fontWeight: activeView === 'intakes' ? '600' : '500', fontSize: '15px', transition: 'all 0.2s' }}
                >
                  <Bell size={20} color={activeView === 'intakes' ? 'var(--ibes-red)' : 'currentColor'} weight={activeView === 'intakes' ? 'fill' : 'regular'} /> Intakes
                </button>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
                    ...
                    {activeView === 'dashboard' ? 'Overview' :
                      activeView === 'manageLeaders' ? 'Personnel' :
                        activeView === 'programmes' ? 'Programmes' :
                          activeView === 'programmeModules' ? 'Programme Modules' :
                            activeView === 'intakes' ? 'Intakes' :
            ...
            <main style={{ flex: 1, padding: isMobile ? '20px' : '40px', overflowY: 'auto' }}>
              ...
              {activeView === 'manageLeaders' && <AdminLeaderManagement leaders={leaders} setLeaders={setLeaders} programmes={programmes} applications={applications} />}
              ...
              {activeView === 'programmeModules' && <AdminModuleManagement modules={programmeModules} setModules={setProgrammeModules} programmes={programmes} />}
              {activeView === 'intakes' && <AdminIntakeManagement intakes={intakes} setIntakes={setIntakes} programmes={programmes} modules={programmeModules} />}
              {activeView === 'leaderFormsSummary' && <AdminFormSummary applications={applications} setApplications={setApplications} type="Leader Module" setActiveView={setActiveView} setVerifyAppId={setVerifyAppId} intakes={intakes} modules={programmeModules} />}
              {activeView === 'tutorFormsSummary' && <AdminFormSummary applications={applications} setApplications={setApplications} type="Teacher Applicant" setActiveView={setActiveView} setVerifyAppId={setVerifyAppId} intakes={intakes} modules={programmeModules} />}
            </main>
          </div>
        </div>
      ) : (
